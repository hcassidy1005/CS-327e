...
def run(argv=None):	
	...
	with beam.Pipeline(options=pipeline_options) as p:
		...
		table_name = "brogrammers-193119:zillow.Rental_Price_3Bedroom" # format: project_id:dataset.table
		table_schema = init_bigquery_table()
    
		lines = p | 'ReadFile' >> beam.io.ReadFromText('gs://cs327e18/zillow/Zip_MedianRentalPrice_3Bedroom.csv')
		...
		tuple_records | 'WriteTmpFile2' >> beam.io.WriteToText('gs://cs327e18/tmp/tuple_records', file_name_suffix='.txt')

		bigquery_records = tuple_records | 'CreateBigQueryRecord' >> beam.Map(create_bigquery_record)
	
		bigquery_records | 'WriteTmpFile3' >> beam.io.WriteToText('gs://cs327e18/tmp/bq_records', file_name_suffix='.txt')
